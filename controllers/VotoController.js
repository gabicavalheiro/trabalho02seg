...
export const votoIndex = async (req, res) => {
  try {
    ...
    const votos = await dbKnex.select("v.*", "t.nome as treino")
      .from("votos as v")
      .innerJoin("treino as t", { "v.treino_id": "t.id" })
...
async function send_email(nome, email, hash) {
  ...
  let info = await transporter.sendMail({
    from: '"Concurso Melhor treino da semana" <MelhorTreino2023@email.com>', // sender address
...
export const votoStore = async (req, res) => {
  ...
  const { nome, email, treino_id } = req.body

  // se não informou estes atributos
  if (!nome || !email || !treino_id) {
    res.status(400).json({ id: 0, msg: "Erro... informe nome, email e treino_id do voto" })
    return
  }
  ...
  const hash = md5(email + treino_id + Date.now())

  try {
    // insere um registro na tabela de votos
    const novo = await dbKnex('votos').insert({ nome, email, treino_id, hash_conf: hash })
...
export const votoConfirme = async (req, res) => {
  ...
  try {
    ...
    await trx("treinos")
      .where({ id: voto[0].treino_id }).increment({ votos: 1 })
